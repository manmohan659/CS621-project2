...
protected:
  ...
  // GetConfigFile is not strictly needed by the simulation but good practice if
  // attribute is used std::string GetConfigFile (void) const; // Already
  // present in provided snippet

private:
  std::vector<uint32_t>
      m_deficits; // Corresponds to DeficitCounter_i from the image
  std::vector<uint32_t> m_quantums; // Corresponds to Quantum_i from the image
  uint32_t m_lastQueueServed; // Stores the index of the last queue that was
                              // processed in the round-robin scan. Used to
                              // determine where the next scan should begin.
