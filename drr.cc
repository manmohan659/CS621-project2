...
TypeId DRR::GetTypeId(void)
{
  static TypeId tid = TypeId("ns3::DRR")
      ...
      //                StringValue (""),
      //                MakeStringAccessor (&DRR::m_configFile), // Needs
      //                DRR::SetConfigFile to be called if set this way
      //                MakeStringChecker ())
      ;
  return tid;
}
...
DRR::DRR()
    : m_lastQueueServed(0) // Default, will be properly set in SetConfigFile
{
...
bool DRR::SetConfigFile(std::string filename)
{
  ...
  // Ensure the base DiffServ object has the correct number of TrafficClass
  // objects. If GetNTrafficClasses() is 0, we are setting up for the first
  // time. If GetNTrafficClasses() > 0 but different from numQueuesFromFile,
  // it's a reconfiguration. The simulation script calls GetTrafficClass(i)
  // after SetConfigFile, so TCs must exist.

  // Clear existing TrafficClass objects if reconfiguring with a different
  // number of queues. This part depends heavily on how DiffServ manages its
  // m_trafficClasses. A robust DiffServ would have a ClearTrafficClasses() or
  // SetNQueues() method. Assuming AddTrafficClass appends and GetTrafficClass
  // accesses an internal vector.
  if (GetNTrafficClasses() != 0 && GetNTrafficClasses() != numQueuesFromFile)
  {
    ...
    // For now, we proceed, DRR will operate on the first 'numQueuesFromFile'
    // TCs.
  }
  ...
  for (uint32_t i = 0; i < numQueuesFromFile; ++i)
  {
    ...
    if (configFileStream.fail() ||
        m_quantums[i] == 0) // Quantum must be positive
    {
  ...
  // Initialize m_lastQueueServed so that the first queue to be checked by
  // Schedule() is queue 0. If numQueuesFromFile is 1, (1-1)=0. Next is
  // (0+1+0)%1 = 0. Correct. If numQueuesFromFile > 1, (numQueuesFromFile-1).
  // Next is ( (N-1)+1+0 )%N = 0. Correct.
...
Ptr<Packet> DRR::Schedule(void)
{
  ...
  // Iterate up to numManagedQueues times to find a queue to service in this
  // round-robin pass
  for (uint32_t i = 0; i < numManagedQueues; ++i)
  {
    ...
    if (!tc)
    { // Should not happen if SetConfigFile was successful
    ...
    // Its deficit should be 0 if it emptied correctly following the image's
    // logic.
    if (tc->IsEmpty())
    {
    ...
    // This queue (currentQueueIndex) is non-empty. This corresponds to "flow i"
    // from the image. This is its "turn". Add its quantum to its deficit. This
    // matches: "DeficitCounter_i = Quantum_i + DeficitCounter_i"
    ...
    NS_LOG_DEBUG(
        ...
        << (m_deficits[currentQueueIndex] -
            m_quantums[currentQueueIndex]) // Deficit before adding quantum
    ...
    //        "  Else break"
    // Since Schedule() in ns-3 typically sends one packet per call, we check
    // once here. If a packet is sent, we update m_lastQueueServed and return.
    // If cannot send (packet too large), its accumulated deficit (OldDeficit +
    // Quantum) is carried over. The 'for' loop continues to check other queues
    // in this *same* Schedule() invocation.

    // Check if queue is still not empty (it shouldn't have changed) and if
    // deficit is positive
    if (!tc->IsEmpty() && m_deficits[currentQueueIndex] > 0)
    {
      ...
      if (packetSize <= m_deficits[currentQueueIndex])
      {
        ...
        m_lastQueueServed = currentQueueIndex; // Update to the queue that
      ...
      {
        // Packet too large for current deficit. Deficit (OldDeficit + Quantum)
        // is carried over. Image: "Else break (*skip while loop*)" for this
        // queue's turn. This queue has had its chance for this quantum,
        // couldn't use it for *this* packet. Its deficit (which includes the
        // just-added quantum) is preserved.
        NS_LOG_DEBUG(
            "DRR: Queue "
        ...
        // m_lastQueueServed is NOT updated here, as this queue didn't
        // successfully send. It will be updated only if a queue sends, or after
        // a full unsuccessful scan.
      }
    }
    else // Queue became empty unexpectedly, or deficit was non-positive after
         // adding quantum (unlikely with uint32_t)
    {
      NS_LOG_DEBUG(
          "DRR: Queue "
          << currentQueueIndex
  ...
  // The last `currentQueueIndex` checked in the loop would be
  // `(m_lastQueueServed_at_entry + 1 + (N-1)) % N` which simplifies to
  // `(m_lastQueueServed_at_entry + N) % N = m_lastQueueServed_at_entry`. So,
  // m_lastQueueServed remains unchanged if a full scan yields no packet, which
  // is correct.
